...
const INSTAGRAM_STATS_TITLE_PATTERN = /^\s*[\d.,\s]+[kKmMbB]?\s+likes?[,;]?\s*[\d.,\s]+[kKmMbB]?\s+comments?(?:\u2026|\.{3})?\s*$/i;
const INSTAGRAM_FULL_TITLE_PATTERN = /^[\d.,]+[kKmMbB]?\s+likes?[,;]?\s*[\d.,]+[kKmMbB]?\s+comments?\s*-\s*(.+?)\s+on\s+.+:\s*[\u201C\u201D"']?([\s\S]+?)[\u201C\u201D"']?\s*$/i;
...
function getMarkdownLinkRanges(title) {
    const ranges = [];
...
function excerptFromText(raw, maxLength = 25) {
...
function extractInstagramCaptionFromDescription(raw) {
...
function isInstagramStatsTitle(title) {
    return INSTAGRAM_STATS_TITLE_PATTERN.test(title);
}
function extractFromInstagramFullTitle(title) {
...
function normalizeHtmlTitle(raw) {
    return decodeHtmlEntities(raw).replace(/\s+/g, " ").trim();
}
function isMeaningfulYouTubeTitle(title) {
...
async function resolveTwitterLabel(url, fetchLike) {
    try {
...
async function resolveYouTubeLabel(url, fetchLike) {
    try {
...
async function resolveUrlLabel(url, fetchLike) {
    try {
        ...
        const descriptionMatch = html.match(OG_DESCRIPTION_PATTERN)?.[1] ?? html.match(META_DESCRIPTION_PATTERN)?.[1];
        ...
        if (YOUTUBE_DOMAINS.has(parsedUrl.hostname)) {
            ...
            if (ogTitleMatch)
                return normalizeHtmlTitle(ogTitleMatch);
        ...
        if (INSTAGRAM_DOMAINS.has(parsedUrl.hostname)) {
            ...
            if (descriptionMatch) {
                ...
                if (fromFullDescription)
                    return fromFullDescription;
                const excerpt = excerptFromText(descriptionMatch, 100);
                if (excerpt && !isInstagramStatsTitle(excerpt))
                    return excerpt;
            }
            if (ogTitleMatch) {
...
export async function normalizeTaskTitleLinks(title, fetchLike = (input, init) => fetch(input, init)) {
    const markdownLinkRanges = getMarkdownLinkRanges(title);
    const urlLabelCache = new Map();
    const replacements = [];
    for (const match of title.matchAll(BARE_URL_PATTERN)) {
        const start = match.index ?? 0;
        if (isInRange(start, markdownLinkRanges))
            continue;
        const raw = match[0];
...
//# sourceMappingURL=normalizeTaskTitleLinks.js.map