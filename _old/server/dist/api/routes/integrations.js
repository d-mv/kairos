...
export async function integrationRoutes(fastify) {
    ...
    fastify.get("/google/callback", { config: { skipAuth: true } }, async (req, reply) => {
        ...
        if (!query.code || !query.state) {
            return reply.redirect(container.googleOAuth.getClientRedirectUrl("error", "google", "Missing Google OAuth parameters"));
        }
    ...
    fastify.delete("/:provider", async (req, reply) => {
        const params = disconnectProviderSchema.safeParse(req.params.provider);
        if (!params.success) {
