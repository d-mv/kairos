...
export async function buildApp() {
    ...
    fastify.get("/ws", { websocket: true, config: { skipAuth: true } }, async (socket, req) => {
        ...
        socket.on("message", async (rawMessage) => {
            ...
            const userId = await resolveUserIdFromToken(token, jwtSecret, process.env["SUPABASE_URL"] ?? "", apiKeyRepo);
            if (!userId) {
                socket.close();
                return;
            }
            ...
            eventBus.addClient(userId, socket);
        });
        socket.on("close", () => {
            clearTimeout(closeTimer);
    ...
    fastify.get("/.well-known/oauth-protected-resource/mcp", { config: { skipAuth: true } }, async (_request, reply) => reply.code(404).send({ error: "Not Found" }));
    fastify.get("/.well-known/oauth-authorization-server", { config: { skipAuth: true } }, async (_request, reply) => reply.code(404).send({ error: "Not Found" }));
    fastify.get("/mcp/.well-known/oauth-authorization-server", { config: { skipAuth: true } }, async (_request, reply) => reply.code(404).send({ error: "Not Found" }));
